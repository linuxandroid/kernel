...
#define LINK_UP_TIMEOUT_US_MAX                5001
...
int msm_pcie_enable(struct msm_pcie_dev_t *dev, u32 options)
{
	...
	if (dev->rc_idx == 1) {
		PCIE_INFO(dev, "optimized link training for RC%d\n",
				dev->rc_idx);
...
int msm_pcie_recover_config(struct pci_dev *dev)
{
	...
	if (msm_pcie_confirm_linkup(pcie_dev, true, true)) {
		...
		PCIE_DBG(pcie_dev, "saving config state for RC%d and its EP\n", pcie_dev->rc_idx);
		pci_save_state(pcie_dev->dev);
		pcie_dev->saved_state = pci_store_saved_state(pcie_dev->dev);
		pci_save_state(dev);
	} else {
		PCIE_ERR(pcie_dev,
